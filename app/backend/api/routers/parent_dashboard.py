...
async def get_parent_overview(student_id:str):
    ...
    pipeline=[{"$match":{"student_id":student_id,"events":{"$exists":True},"session_duration_seconds":{"$type":"number"}}},
              {"$group":{"_id":"$session_id","seconds":{"$max":"$session_duration_seconds"}}},
              {"$group":{"_id":None,"seconds":{"$sum":"$seconds"}}}]
    durations=await db.telemetry_events.aggregate(pipeline).to_list(length=1)
...
async def get_parent_activity_history(student_id:str,limit:int=Query(10,ge=1,le=50),days:int|None=Query(None,ge=1,le=365)):
    ...
    history=[]
    for r in rows:
        raw=await db.telemetry_events.find_one({"student_id":student_id,"session_id":r.get("session_id"),"events":{"$exists":True}})
